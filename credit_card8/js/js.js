$(function() {
	...
	$('.record').click(function() {
		...
		if(total_value == 0 || total_value == "" || total_value == Infinity || alike == total_value||total_value < 0||total_value =="不需要刷卡") {
			alert("记录失败！请重新输入")
		} else {
			alike = total_value
	...
	for(var i = 0; i < ka_nanesky.length; i++) {
		...
		if(momentsky != null) {
			if(momentsky<=0){
				...
				//强制保留两位小数  toFixed(2)
				mean_days = (days_sky/effective_date).toFixed(0)
				//			console.log(mean_days)
				//			console.log(days_sky)
				//			console.log(effective_date)
			}
		}

	}
	localStorage.setItem("days_sky", mean_days);
	localStorage.setItem("effective_date", effective_date+"有效日期个数");
	var erer = localStorage.getItem("effective_date");
	console.log(erer)
	
	$('.dayss').val(mean_days + "天")
	//总金额计算与存储--------------------------	
...
function count() {
	...
	var refund_gross = localStorage.getItem("refund_gross");
	var eter = Number(refund_gross)
//	console.log(eter)
		if(already>eter){
			$('.rental').html("不需要刷卡")
		}else{
